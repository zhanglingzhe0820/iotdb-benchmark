...
//		}
    ...
    if (config.READ_FROM_FILE) {
      ...
      long pointsOneLoop =
          config.DEVICE_NUMBER / config.CLIENT_NUMBER * config.SENSOR_NUMBER * config.BATCH_SIZE;
      ...
      while (i < config.LOOP) {
        ...
        if (config.BENCHMARK_WORK_MODE.equals(Constants.MODE_INSERT_TEST_WITH_USERDEFINED_PATH)) {
          try {
            database
                .insertGenDataOneBatch(config.STORAGE_GROUP_NAME + "." + config.TIMESERIES_NAME, i,
...
//              LOGGER.info("进入数据顺序插入模式");
          ...
          try {
            for (int m = 0; m < clientDevicesNum; m++) {
              maxIndex = database
                  .insertOverflowOneBatch(config.DEVICE_CODES.get(index * clientDevicesNum + m),
                      i,
                      totalTime,
                      errorCount,
                      before,
          ...
          try {
            for (int m = 0; m < clientDevicesNum; m++) {
              currMaxIndexOfDist = database
                  .insertOverflowOneBatchDist(config.DEVICE_CODES.get(index * clientDevicesNum + m),
                      i,
                      totalTime,
                      errorCount,
                      currMaxIndexOfDist,
...
//        LOGGER.info("执行一个batch的总时间为{}", loopDeltaTime);
        ...
        /*
        LOGGER.info("{} LOOP ,{}, INSERT RATE,{},points/s,LOOP DELTA TIME,{},second",
            Thread.currentThread().getName(), i, loopRate, loopSecond);*/
