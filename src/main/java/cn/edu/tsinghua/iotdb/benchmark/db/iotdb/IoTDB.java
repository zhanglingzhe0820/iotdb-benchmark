...
public class IoTDB implements IDatebase {
  ...
  private static final String insertText = "insert into %s(timestamp,raw_data) values(%s,'%s')";
  ...
  public void createSchema() throws SQLException {
    if (config.READ_FROM_FILE) {
      ...
      for (String device : config.DEVICE_CODES) {
        ...
        path = getGroupDevicePath(device);
        statement.addBatch(String.format(createSeriesSQLWithCompressor,
            Constants.ROOT_SERIES_NAME + "." + path + ".raw_data",
            "TEXT", "PLAIN", config.COMPRESSOR));
  ...
  public void insertOneBatchMulDevice(LinkedList<String> deviceCodes, int loopIndex,
    ...
    try {
      ...
      mySql.saveInsertProcess(loopIndex, costTime / unitTransfer, totalTime.get() / unitTransfer,
          errorNum, config.REMARK);
  ...
  private long executePreparedStatement(int loopIndex, int batchIndex, String device,
    ...
    statement.execute(String
        .format(insertText, Constants.ROOT_SERIES_NAME + "." + path, currentTime,
            Constants.TEXT_GEN));
      ...
      if (!config.IS_OVERFLOW) {
        if (config.USE_PREPARE_STATEMENT) {
          ...
          for (int i = 0; i < config.BATCH_SIZE; i++) {
            try {
              genTime += executePreparedStatement(loopIndex, i, device, statement);
            } catch (Exception e) {
    ...
    try {
      ...
      switch (config.QUERY_CHOICE) {
        case 1:// 精确点查询
          ...
          long timeStamp =
              (startTime - Constants.START_TIMESTAMP) / config.POINT_STEP * config.POINT_STEP
        ...
        case 8:// query with limit and series limit and their offsets
          ...
          sql = createQuerySQLStatment(device_id, config.QUERY_LIMIT_N, config.QUERY_LIMIT_OFFSET,
              config.QUERY_SLIMIT_N, config.QUERY_SLIMIT_OFFSET);
          break;
...
//			LOGGER.info("{}",builder.toString());
    ...
    try {
      ...
      if (config.READ_FROM_FILE) {
        ...
        statement.execute(String.format(createSeriesSQLWithCompressor,
          ...
          .equals(Constants.MODE_INSERT_TEST_WITH_USERDEFINED_PATH)) {
        statement.execute(String.format(createSeriesSQLWithCompressor,
        ...
        writeSQLIntoFile(String.format(createSeriesSQLWithCompressor,
            path + "." + sensor, config.TIMESERIES_TYPE, config.ENCODING, config.COMPRESSOR),
            config.GEN_DATA_FILE_PATH);
      } else {
        statement.execute(String.format(createSeriesSQLWithCompressor,
            Constants.ROOT_SERIES_NAME + "." + path + "." + sensor, config.DATA_TYPE,
            config.ENCODING, config.COMPRESSOR));
      }
      statement.close();
    } catch (SQLException e) {
  ...
  private void createTimeseriesBatch(String path, String sensor, int count, int timeseriesTotal,
    ...
    if ((count % 1000) == 0) {
      ...
      mySql.saveResult("batch" + count / 1000 + "CreateTimeseriesSpeed",
          "" + 1000000000000.0f / (endTime - startTime));
  ...
  private void setStorgeGroup(String group) {
    ...
    try {
      ...
      if (config.READ_FROM_FILE) {
        ...
        writeSQLIntoFile(String.format(setStorageLevelSQL, config.STORAGE_GROUP_NAME),
            config.GEN_DATA_FILE_PATH);
      } else {
        statement
            .execute(String.format(setStorageLevelSQL, Constants.ROOT_SERIES_NAME + "." + group));
      }
  ...
  public String createSQLStatment(int batch, int index, String device) {
    StringBuilder builder = new StringBuilder();
    String path = getGroupDevicePath(device);
    builder.append("insert into ").append(Constants.ROOT_SERIES_NAME).append(".").append(path)
        .append("(timestamp");
    for (String sensor : config.SENSOR_CODES) {
   ...
   * 创建查询语句--(精确点查询)
   */
  public String createQuerySQLStatment(List<Integer> devices, int num, long time,
   ...
   */
  public String createQuerySQLStatment(List<Integer> devices, int num, List<String> sensorList)
      throws SQLException {
    StringBuilder builder = new StringBuilder();
   ...
   * 创建查询语句--(带有聚合函数的查询)
   */
  public String createQuerySQLStatment(List<Integer> devices, int num, String method,
      List<String> sensorList) {
    StringBuilder builder = new StringBuilder();
  ...
  public String createQuerySQLStatment(List<Integer> devices, int num, String method,
    ...
    StringBuilder builder = new StringBuilder(
        createQuerySQLStatment(devices, num, method, sensorList));
  ...
  public String createQuerySQLStatment(List<Integer> devices, int num, long startTime, long endTime,
    ...
    StringBuilder builder = new StringBuilder();
    builder.append(createQuerySQLStatment(devices, num, startTime, endTime, value, sensorList))
        .append(" limit ").append(limit_n);
   ...
   * 创建查询语句--(带有limit条件的查询)
   */
  public String createQuerySQLStatment(int device_id, int limit_n, int offset, int series_limit,
      int series_offset) {
    StringBuilder builder = new StringBuilder();
    builder.append("SELECT *");
   ...
   */
  public String createQuerySQLStatment(List<Integer> devices, String method, int num,
      List<Long> startTime, List<Long> endTime, Number value,
      List<String> sensorList) throws SQLException {
    StringBuilder builder = new StringBuilder();
    builder.append(createQuerySQLStatment(devices, num, method, sensorList));
//        builder.append(" WHERE ");
...
//        }
//        builder.delete(builder.lastIndexOf("AND"), builder.length());
    builder.append(" GROUP BY(").append(config.TIME_UNIT).append("ms, ")
        .append(Constants.START_TIMESTAMP);
  ...
  private String getFullGroupDevicePathByID(int id) {
    ...
    int hashCode = device.hashCode();
    return Constants.ROOT_SERIES_NAME + ".group_" + (Math.abs(hashCode) % config.GROUP_NUMBER) + "."
        + device;
  }
  ...
  @Override
  public void insertGenDataOneBatch(String device, int loopIndex, ThreadLocal<Long> totalTime,
      ThreadLocal<Long> errorCount, ArrayList<Long> latencies) throws SQLException {
    Statement statement;
    int[] result;
    long errorNum = 0;
...
//				}
              ...
              LOGGER.info(
                  "Execute one SQL from file, resultSet size is {}, it costs {} ms, current total time {} ms, the SQL : {}",
                  lines,
                  costTime / 1000000.0,
          ...
          LOGGER.info(
              "Execute {} SQL from file finished, it costs {} seconds, mean rate {} SQL/s . {} SQL execute failed.",
              count,
  ...
  static public void main(String[] args) throws SQLException {
    ...
    //聚合函数
    sql = ioTDB
        .createQuerySQLStatment(devices, 3, "max_value", 1000000, 1000000 + config.QUERY_INTERVAL,
            sensorList);
    //不带filter的聚合查询
